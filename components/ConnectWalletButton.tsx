...
export const ConnectWalletButton = () => {
    ...
    return (
        ...
        </div> : <ConnectedButton address={address!} disconnect={() => disconnect}/>}
...
const ConnectedButton = (props: {address: string,disconnect: any} ) => {
    ...
    return (
        <>
    <div className="connectWalletButton" onClick={() => setShowDropDown(!showDropDown)}>
        {shortenAddress(props.address)} &nbsp; <picture><img src="/downArrow.svg" alt="" style={showDropDown ? {transform:"rotateX(180deg)"} : {}}></img></picture>
    </div>
        {showDropDown && <div className={styles.container}>
                ...
                <div className={styles.dropdown}>
                    ...
                    <div className={styles.addressContainer}>
                        <div>{shortAddressForModal(props.address)}</div>
                            <picture className={styles.copyIcon} onClick={() => CopyAddress(props.address)}>
                                <img src="/copyIcon.svg" ></img>
                            </picture> 
                        </div>
                    <TransactionList/>
                    <div className={styles.disconnect} onClick={props.disconnect()}>disconnect wallet</div>
                </div>
